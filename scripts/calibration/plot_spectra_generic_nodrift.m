function plot_spectra_generic_nodrift(calibrationTool, drift, meteoData, calibratedSpectra, N)
%==========================================================================
...
try 
    ...
    if ~isempty(drift)
        ...
        ax = subplot(3,2,1); plot([calibratedSpectra.meanAntTime],[calibratedSpectra.TNoise],'r'), hold on, ylabel('T_N [K]') , xlim([xstart,xstop]), ylim([limTNPlot-200,limTNPlot+200])
        plot(ax, [calibratedSpectra.meanAntTime], limTNPlot*[calibratedSpectra.outlierCalib]-100,'mx'), xlim([xstart,xstop]);
        
        ax2 = subplot(3,2,2); plot(ax2, [calibratedSpectra.meanAntTime], [calibratedSpectra.meanTb] ,'g'), hold on, ylabel('T_a [K]'), xlim([xstart,xstop]), ylim([0,medianTa+120])
        ...
        plot([calibratedSpectra.meanAntTime], [calibratedSpectra.meanHotCounts],colors{1}), hold on, ylabel('Counts [-]'), xlim([xstart,xstop])
        plot([calibratedSpectra.meanAntTime], [calibratedSpectra.meanSkyCounts],colors{2}), hold on, ylabel('Counts [-]'),xlim([xstart,xstop])
        plot([calibratedSpectra.meanAntTime], [calibratedSpectra.meanColdCounts],colors{3}), hold on, ylabel('Counts [-]'), xlim([xstart,xstop])
