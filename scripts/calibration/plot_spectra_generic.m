...
if ~isempty(drift)
    ...
    if ~isempty(drift.outlierCold)
        plot(ax, drift.outlierCold,nanmedian(drift.Tn)-50,'bx')
        plot(ax2, drift.outlierCold,10,'bx')
    end
    if ~isempty(drift.outlierHot)
        plot(ax, drift.outlierHot,nanmedian(drift.Tn)-50,'rx')
        plot(ax2, drift.outlierHot,10,'rx')
    end
    if ~isempty(drift.outlierSky)
        plot(ax, drift.outlierSky,nanmedian(drift.Tn)-50,'gx')
        plot(ax2, drift.outlierSky,10,'gx')
    end
