...
try 
    ...
    if ~isempty(drift)
        ...
        if ~isempty(drift.outlierCold) && length(drift.outlierCold) < 100
            plot(ax, drift.outlierCold,nanmedian(drift.Tn)-40,'bx')
            plot(ax2, drift.outlierCold,10,'bx')
        end
        if ~isempty(drift.outlierHot) && length(drift.outlierHot) < 100
            plot(ax, drift.outlierHot,nanmedian(drift.Tn)-50,'rx')
            plot(ax2, drift.outlierHot,20,'rx')
        end
        if ~isempty(drift.outlierSky) && length(drift.outlierSky) < 100
            plot(ax, drift.outlierSky,nanmedian(drift.Tn)-60,'gx')
            plot(ax2, drift.outlierSky,30,'gx')
        end
