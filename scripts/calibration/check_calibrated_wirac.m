function [calibratedSpectra, logFile] = check_calibrated_wirac(logFile,calibrationTool,calibratedSpectra)
%==========================================================================
...
for i = 1:size(calibratedSpectra,2)
    ...
    % In this case, we overwrite frequency vector.
    f=linspace(-1,1,16385); f(1)=[]; 
    calibratedSpectra(i).if = [f*100 f*500];
    %calibratedSpectra(i).if = [100*linspace(-1,1,16385) 500*linspace(-1,1,16385)];
    %calibratedSpectra(i).if(1)=[];
    %calibratedSpectra(i).if(16385)=[];
%   calibratedSpectra(i).observationFreq=calibrationTool.observationFreq;
%         
    calibratedSpectra(i).freq=calibratedSpectra(i).if*1e6+ calibratedSpectra(i).observationFreq+10e6;
    ...
    YcenterSmoothed=Ycenter(abs(Ycenter-Yfiltered)<2*std(Ycenter));%nan to without nan
    ...
    calibratedSpectra(i).TNoise=mean(TNoiseCenter);%nan to without nan
    ...
    calibratedSpectra(i).TNoiseLog=mean(logFile.FE_T_Sys(ind)); %nan to without nan
    calibratedSpectra(i).stdTNoiseLog=std(logFile.FE_T_Sys(ind));%nan to without nan
    ...
    calibratedSpectra(i).meanStdTb=mean(calibratedSpectra(i).stdTb(~calibratedSpectra(i).potentialBadChannels)); %nan to without nan
    
    diff_Tb = diff(calibratedSpectra(i).Tb(~calibratedSpectra(i).potentialBadChannels));
    calibratedSpectra(i).noiseLevel = std(diff_Tb(~isinf(diff_Tb)))/sqrt(2);%nan to without nan

    % Mean calibrated Tb for this cycle:
    calibratedSpectra(i).meanTb = mean(calibratedSpectra(i).Tb(~calibratedSpectra(i).potentialBadChannels));%nan to without nan
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Cold and Hot spectra variation among this cycle
    calibratedSpectra(i).meanStdHotSpectra=mean(calibratedSpectra(i).stdHotSpectra);%nan to without nan
    calibratedSpectra(i).meanStdColdSpectra=mean(calibratedSpectra(i).stdColdSpectra);%nan to without nan
    
    % Mean counts
    calibratedSpectra(i).meanHotCounts = mean(calibratedSpectra(i).meanHotSpectra);%nan to without nan
    calibratedSpectra(i).meanColdCounts = mean(calibratedSpectra(i).meanColdSpectra);%nan to without nan
    calibratedSpectra(i).meanSkyCounts = mean(calibratedSpectra(i).meanSkySpectra);%nan to without nan
...
%     
    ...
    if isfield(logFile,'T_Room')
        calibratedSpectra(i).TempRoom=mean(logFile.T_Room(ind));%nan to without nan
        calibratedSpectra(i).stdTempRoom=std(logFile.T_Room(ind));%nan to without nan
    else
        calibratedSpectra(i).TempRoom=-9999;
        calibratedSpectra(i).stdTempRoom=-9999;
    end
    if isfield(logFile,'T_Out')
        calibratedSpectra(i).TempOut=mean(logFile.T_Out(ind));%nan to without nan
        calibratedSpectra(i).stdTempOut=std(logFile.T_Out(ind));%nan to without nan
    else
        calibratedSpectra(i).TempOut=-9999;
        calibratedSpectra(i).stdTempOut=-9999;
    end
    if isfield(logFile,'T_Window')
        calibratedSpectra(i).TempWindow=mean(logFile.T_Window(ind));%nan to without nan
        calibratedSpectra(i).stdTempWindow=std(logFile.T_Window(ind));%nan to without nan
    else
    ...
    if ~isempty(ia)
        ...
        calibratedSpectra(i).meanAntTime = mean(logFile.dateTime(ia));%nan to without nan
        calibratedSpectra(i).meanAntTimeUTC = mean(logFile.dateTime(ia));%nan to without nan
    ...
    if isfield(logFile,'TC')
       ...
       if sum(isTC) > 0
           ...
           logFile.TC(isTC).coldCalib = mean(logFile.TC(isTC).coldSpectra);%nan to without nan
           logFile.TC(isTC).hotSpectra = calibratedSpectra(i).meanHotSpectra(logFile.TC(isTC).channels);
           logFile.TC(isTC).hotCalib = mean(logFile.TC(isTC).hotSpectra);%nan to without nan
           ...
           TC(isTC).TbRJE_mean = mean(logFile.TC(isTC).TbRJE,2);%nan to without nan
...
%     end
   % calibratedSpectra(i).effectiveCalibrationTime=effectiveTime;
    ...
    calibratedSpectra(i).errorVector=[sufficientNumberOfIndices,1,1,1,1,1,1];
