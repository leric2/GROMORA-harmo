function calibratedSpectra = check_calibrated_miawara_c(standardLog,calibrationTool,calibratedSpectra)
%==========================================================================
...
for i = 1:size(calibratedSpectra,2)
    ia=find(calibratedSpectra(i).antennaIndCleanAngle);
    ih=calibratedSpectra(i).hotInd;
    ic=calibratedSpectra(i).coldInd;
    ir=calibratedSpectra(i).refInd;
    
    if ~isempty(ia) & ~isempty(ih) & ~isempty(ic) & ~isempty(ir)
        ind=sort([ia ih ic ir]);
    ...
    %    calibratedSpectra(i).df=calibrationTool.samplingRateFFTS/(2*calibrationTool.numberOfChannels);
        ...
        calibratedSpectra(i).freq = interp1(f(:,1)',f(:,2)',1:N);%N/2
    ...
    %     centerChannels=(calibratedSpectra(i).freq >= calibratedSpectra(i).observationFreq-200e6 & calibratedSpectra(i).freq<calibratedSpectra(i).observationFreq+200e6);
    %     
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        timeMin = hours(calibratedSpectra(i).theoreticalStartTime) + datetime(calibratedSpectra(i).year, calibratedSpectra(i).month, calibratedSpectra(i).day);
        timeMax = hours(calibratedSpectra(i).theoreticalStartTime) + minutes(calibratedSpectra(i).calibrationTime) + datetime(calibratedSpectra(i).year, calibratedSpectra(i).month, calibratedSpectra(i).day);
        timeMean =  hours(calibratedSpectra(i).theoreticalStartTime) + minutes(calibratedSpectra(i).calibrationTime)/2 + datetime(calibratedSpectra(i).year, calibratedSpectra(i).month, calibratedSpectra(i).day);

        calibratedSpectra(i).timeMin = datenum(timeMin-datenum(1970,1,1))*86400;
        calibratedSpectra(i).timeMax = datenum(timeMax-datenum(1970,1,1))*86400;
        calibratedSpectra(i).timeMean = datenum(timeMean-datenum(1970,1,1))*86400;
...
%                 disp('Irregularity in the calculation of the effective time in "check_calibrated_miawara_c"')
%             end
%         else
%             % TODO
%             effectiveTime=effectiveTime;
%         end
        ...
        calibratedSpectra(i).numberOfIndices=[length(unique(ih)) length(unique(ic)) length(unique(ia)) length(ir)];
