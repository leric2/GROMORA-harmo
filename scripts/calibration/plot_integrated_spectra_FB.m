function plot_integrated_spectra_FB(calibrationTool,integratedSpectra)
%==========================================================================
...
%     ylabel(ax3(1),({'airP [hPa]'}))
    ...
    cmax = max([integratedSpectra.numberOfAveragedSpectra]);
...
%    title('Corrected (windows), good channels')
    ...
    subplot(3,2,5); 
    for i=1:N
        plot(integratedSpectra(i).intermediate_freq,integratedSpectra(i).Tb.*integratedSpectra(i).channelsQuality,'-', 'Color',cm(i,:));
    ...
    title('Non Corrected, good channels')
    grid on
