...
from pyarts.retrieval import RetrievalQuantity as RetQPyarts
    ...
    def to_xarray(self):
        ...
        ds = xr.Dataset(
            data_vars={
                ...
                prefix + 'avkm': ((flat_grid_name, flat_grid_name+ '_avk'), self.avkm),#+ '_avk'  
            },
            coords=coords,
            attrs={
...
class GriddedRetrievalQuantity(RetrievalQuantity):
    ...
    def to_xarray(self):
        ...
        if self.dimensions == 1:
            ...
            # ds.drop_vars([prefix+'lat', prefix+'lon'])
            # ds.reset_coords(prefix+'lat', drop=True)
            # dims ={'dim1':prefix+'lat', 'dim0':prefix+'lon'} 
    ...
    @property
    def grids(self):
        return [self.p_grid, self.lat_grid, self.lon_grid] 
...
class FreqShift(RetrievalQuantity):
    ...
    def grid_names(self):
        """Names of the grids."""
        return ['f_shift_grid','observation']
...
class Polyfit(RetrievalQuantity):
    ...
    def apply(self, ws):
        ...
        jqs = ws.jacobian_quantities.value
        self._sequence_id =  slice(len(jqs) - n_jqs, len(jqs)) # slice(len(jqs) - n_jqs, len(jqs) - n_jqs+1)# len(jqs))
        self._jacobian_quantity = jqs# [self._sequence_id]#  jqs[2] #
        self._slice = _jqs_slices(jqs)[self._sequence_id]
        self._is_applied = True
    ...
    def grids(self):
       ...
       #     raise NotImplementedError()
        return [np.arange(self.poly_order + 1),[1]]  # [np.arange(self.poly_order + 1), np.array(jq_grids[-1], dtype=np.int)]
...
class Sinefit(RetrievalQuantity):
    ...
    def __init__(self, periods, covmats, pol_variation=False, los_variation=False, mblock_variation=False):
        if len(covmats) != len(periods):
            raise ValueError('Must provide len(periods) covariance matrices.')
        for covmat in covmats:
            if len(covmat) != 2:
                raise ValueError('Sinefit covariance matrices must have ndim=2, but got {}.'.format( len(covmat) ))
    ...
    def apply(self, ws):
        ...
        n_jqs = len(self._periods)
        jqs = ws.jacobian_quantities.value
        self._first_id = len(jqs) - n_jqs
    ...
    def apply_covmat(self):
        ws = self.ws
        for cov in self._covmats:
        # for i in self._periods:
           # self.ws.covmat_sxAddBlock(block=covmat)
            set_variable_by_xml(ws, ws.covmat_block, sparse.diags(cov))
            ws.covmat_sxAddBlock(block=ws.covmat_block)
    ...
    def to_xarray_single(self, i, p):
        grid_names = ('sine_grid', 'observation')
        grids =self.grids #  [np.array([0,1],dtype=np.int), [1] ]  #   self.grids[[1,1] [1]]
        coords = {n: c for n, c in zip(grid_names, grids)}  ## {grid_names, tuple(grids)}#  

        prefix = self.slug + '_' + str(i) + '_'
        flat_grid_name = prefix + 'grid'

        ds = xr.Dataset(
            data_vars={
                prefix + 'x': (grid_names,np.split(self.x[i],2)),# ),
                #  prefix + 'xa': (grid_names, np.stack(self.xa[i])),
                # prefix + 'mr': (grid_names, np.stack(self.mr[i])),
                # prefix + 'eo': (grid_names, np.stack(self.eo)),
                # prefix + 'es': (grid_names, np.stack(self.es)),
                # prefix + 'avkm': ((grid_names[0], flat_grid_name, flat_grid_name + '_avk'), np.stack(self.avkm)),
            },
    ...
    def to_xarray(self):
        ...
        ds = xr.merge([self.to_xarray_single(i, p) for i, p in enumerate(self._periods)])

       # ds = ds.expand_dims("observation").assign_coords(observation=("observation",1))
    ...
    @property
    def grids(self):
        jq_grids = self._jacobian_quantity[self._first_id].grids
        return [ np.arange(2), [1] ] 
