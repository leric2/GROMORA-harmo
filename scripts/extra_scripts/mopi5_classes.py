...
class MOPI5_LvL2(DataRetrieval):
    ...
    def compare_spectra_binned_interp_mopi5_factor(self, dim='time', idx=[0], save_plot = False, spectrometers=['AC240'], identifier=[], use_basis = 'U5303', alpha=[0,7,8,9], binning=8, lowerBound=[], corr_band=[], variable=False, broadband_bias=[], paper=False, outfolder=None):
        figures = list()
        #spectro_ds = self.calibrated_data[s]
        ...
        if save_plot:
            if outfolder is not None:
                save_single_pdf(outfolder+outName+self.integration_strategy+'_'+self.datestr+'.pdf', figures)
            else:
                save_single_pdf(self.level1_folder+outName+self.integration_strategy+'_'+self.datestr+'.pdf', figures)
    
    def compare_spectra_binned_interp_mopi5(self, dim='time', idx=[0], spectrometers=['AC240','USRP-A'], use_basis='U5303', save_plot = False, identifier=[], corrected=False, clean=False, outfolder=None):
        figures = list()
        #spectro_ds = self.calibrated_data[s]
        for i in idx:
        ...
        if save_plot:
            ...
            if corrected:
                ...
                if outfolder:
                    save_single_pdf(outfolder+'spectra_interp_diff_comparison_'+self.integration_strategy+'_'+self.datestr+'_'+str(idx)+'.pdf', figures)
                else:
